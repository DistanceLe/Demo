...
+(void)getAllGroup:(PHGroupBlock)handler{
    if (![self isHadAuthorization]) {
        [LJAlertView showAlertWithTitle:@"没有授权" message:@"打开系统设置，授权" showViewController:nil cancelTitle:@"取消" otherTitles:@[@"打开"] clickHandler:^(NSInteger index, NSString *title) {
            if (index == 1) {
...
+(void)getRollAlbum:(PHGroupBlock)handler{
    ...
    for (PHAssetCollection *sub in smartAlbums){
        ...
        //[albumName containsString:@"Roll"] || [albumName containsString:@"相机胶卷"]
        if (albumName.length) {
            [alubms addObject:sub];
            [names addObject:sub.localizedTitle];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    if (error) {
        ...
        [LJInfoAlert showInfo:LS(@"CE_SaveSuccess") bgColor:nil];
    }
}
...
+(void)getFileURLFromVideoAsset:(PHAsset*)sourceAsset handler:(PHVideoURLBlock)handler{
    if (sourceAsset && sourceAsset.mediaType == PHAssetMediaTypeVideo) {
        ...
        [LJInfoAlert showInfo:LS(@"CE_GetURLError") bgColor:nil];
    }
}
...
+(void)getHeadImageHandler:(PHImageBlock)handler{
    ...
    [LJSheetAlertView showSheetWithTitles:@[LS(@"CE_TakePhoto"), LS(@"CE_SelectPhotoFromAlbum")] handler:^(NSInteger flag, NSString *title) {
        if (flag==1) {
            if ([tempSelf isCameraAccessShowTips:YES]) {
                 [tempWeakSelf openCameraCanEdit:YES];
            }
            
        }else if (flag==2){
            if ([tempSelf isPhotoLibraryAccessShowTips:YES]) {
...
- (BOOL)isHaveCameraShowTips:(BOOL)show
{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]){
        ...
        if (show) {
            UIAlertView *alertV = [[UIAlertView alloc] initWithTitle:LS(@"Comment_Tip")
                                                             message:@"您的设备没有照相机"
                                                            delegate:nil
                                                   cancelButtonTitle:LS(@"Comment_KnowIt")
...
{
    if (![self isHaveCameraShowTips:show]){
        return NO;
    }
...
- (BOOL)isPhotoLibraryAccessShowTips:(BOOL)show
{
